...
/** @type {Record<string, import('generic-pool').Pool<CodeCaller>> | null} */
let coursePools = null;

/** @type {Set<CodeCaller>} */
const unhealthyCodeCallers = new Set();

/**
 * Track which pool each unhealthy code caller came from, so that we can
 * use the same code to handle both `container` and `dev-container` execution
 * modes.
 *
 * @type {Map<CodeCaller, import('generic-pool').Pool<CodeCaller>>} */
const unhealthyCodeCallerPools = new Map();

async function getHealthyCodeCaller() {
  // eslint-disable-next-line no-constant-condition
  while (true) {
    const codeCaller = await globalPool.acquire();
    if (!unhealthyCodeCallers.has(codeCaller)) {
      return codeCaller;
    }
    await globalPool.release(codeCaller);
    await sleep(0);
  }
}
...
function destroyUnhealthyCodeCallers() {
  unhealthyCodeCallers.forEach((codeCaller) => {
    const pool = unhealthyCodeCallerPools.get(codeCaller);

    // Delete from the set/map first. That way, if `pool.destroy()` is still running
    ...
    unhealthyCodeCallerPools.delete(codeCaller);
...
module.exports = {
  async init() {
    ...
    if (!['container', 'dev-container', 'native', 'disabled'].includes(workersExecutionMode)) {
    ...
    if (workersExecutionMode === 'container' || workersExecutionMode === 'dev-container') {
      await initCodeCallerDocker();
    }

    function createPool(numWorkers, courseDirectory = null) {
      const pool = genericPool.createPool(
        {
          create: async () => {
            ...
            console.log('ensuring child');
            await codeCaller.ensureChild();
            console.log('child ensured');
    ...
    if (config.workersExecutionMode === 'dev-container') {
      coursePools = {};
      config.courseDirs.forEach((courseDir) => {
        // When running in fixed-course mode, only create one worker per course.
        coursePools[courseDir] = createPool(1, courseDir);
      });
    } else {
      const numWorkers = config.workersCount ?? Math.ceil(config.workersPerCpu * os.cpus().length);
      globalPool = createPool(numWorkers);
    }
    ...
    if (config.workersExecutionMode === 'dev-container') {
      await Promise.all(Object.values(coursePools).map((coursePool) => coursePool.ready()));
    } else {
      await globalPool.ready();
    }
  },
  ...
  async finish() {
    ...
    if (config.workersExecutionMode === 'dev-container') {
      await Promise.all(
        Object.values(coursePools).map(async (coursePool) => {
          await coursePool.drain();
          await coursePool.clear();
        })
      );
      coursePools = null;
    } else {
  ...
  async withCodeCaller(coursePath, fn) {
    ...
    const useCoursePool = config.workersExecutionMode === 'dev-container';
    const pool = useCoursePool ? coursePools[coursePath] : globalPool;

    if (!pool) {
      throw new Error(`No pool for course ${coursePath}`);
    }
    ...
    try {
      ...
      unhealthyCodeCallerPools.set(codeCaller, pool);
      throw err;
    } finally {
